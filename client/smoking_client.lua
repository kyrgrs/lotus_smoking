...
function useSmokingItems(itemName)
    ...
    if item.returnItem == true then
        TriggerServerEvent('lotus_smoking:server:returnItems', item.returned, item.amountReturned)
    end
    ...
    -- Vape tipi item'leri kontrol et
    if item.itemType and item.itemType == "vape" then
        TriggerServerEvent('lotus_smoking:server:UseVapeJuice', itemName)
    else
        TriggerServerEvent('lotus_smoking:server:UseItem', itemName)
    end
...
--use smoking item
RegisterNetEvent('lotus_smoking:client:UseItem', function(itemName)
    local item = Config.Consumables[itemName] 
    if not item then return end
    if busy then
        SendNotify(Config.Language.Notifications.Busy, 'error', 2500)
    else
        QBCore.Functions.TriggerCallback('lotus_smoking:server:hasItem', function(hasItem)
            if hasItem then
                busy = true
                LockInventory(true)
...
function openSmokingShop()
    if InvType == 'qb'then
		TriggerServerEvent('lotus_smoking:server:openShop')
